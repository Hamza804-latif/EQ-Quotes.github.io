import { AgentAuthModel, AdminAuthModel } from "../database/models/auth.js";
...
export const SuperAgentAuth = async (req, resp) => {
  let { username, password } = req.body;
  try {
    let agentLogin = await AdminAuthModel.findOne({ username, password });
    if (agentLogin) {
      jsonwebtoken.sign({ username }, process.env.secretKey, (err, token) => {
        if (err) {
          return resp.json({ msg: "something went wrong!" });
